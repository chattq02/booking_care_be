...
import { AcademicTitleRepository } from 'src/repository/admin/academic_title.repo'
...
export class AcademicTitleService {
  ...
  getListAcademicTitle = async (query: GetListAcademicTitleQueryDto, res: Response) => {
    ...
    const { data, total } = await this.academicRepo.findMany(keyword, Number(skip), Number(per_page))
